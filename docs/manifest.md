...
- `gateway.wifi.enabled` opens the gate topology API on the WAN firewall zone.
- `gateway.wifi.enabled` with SSH enabled also opens SSH on the WAN firewall
  zone.
- Gate topology API exposure on WAN (port 10411) is intended for trusted
  upstream Wi-Fi LANs and is sensitive on untrusted uplinks.
